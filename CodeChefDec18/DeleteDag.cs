...
Scoring
The score for each test file is 10⋅(C/S)2, where C is the number of steps in which you deleted exactly 2 vertices. The final score of your submission is equal to the sum of scores for each test file.
...
1 1
 */
namespace CodeChefDec18
{
    internal static class DeleteDag
    {
        private static void MainDAG(string[] args)
        {
            var pars = Console.ReadLine().Split(' ').Select(int.Parse).ToList();
            var N = pars[0];
            var M = pars[1];
            var nodes = new Node[N];
            for (var i = 0; i < M; i++)
            {
                var vertices = Console.ReadLine().Split(' ').Select(int.Parse).ToList();
                var start = vertices[0];
                if (nodes[start] == null)
                {
                    nodes[start] = new Node();
                }
                nodes[start].AddLink(vertices[1]);
            }
        ...
        class Node
        {
            private readonly IList<int> linkedTo = new List<int>();

            public void AddLink(int otherVertex)
            {
                linkedTo.Add(otherVertex);
            }
        }
    }
}