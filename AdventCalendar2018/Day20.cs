...
namespace AdventCalendar2018
{
    ...
    {
        private static void MainDay20()
        {
            var line = /*Console.ReadLine()*/Input;
            var length = new Stack<List<int>>();
            var cumulatedLengths = new Stack<int>();
            var targetLength = 0;
            ...
            foreach (var car in line)
            {
                ...
                {
                    case ')':
                        // we need to process all length
                        var sub = length.Pop();
                        var maxLength = 0;
                        foreach (var part in sub)
                        {
                            if (part == 0)
                            {
                                // optional path
                        ...
                        // we compute how many rooms are beyond the limit
                        var gap = Math.Min(0, currentLength - limit);
                        if (maxLength > 0)
                        {
                            foreach (var roomDist in sub)
                            {
                                numberRoomsBeyondLimit += Math.Max(0, roomDist + gap);
                            }
                        }
                        else
                        {
                            // dead end, only half the dist
                            foreach (var roomDist in sub)
                            {
                                numberRoomsBeyondLimit += Math.Max(0, roomDist/2 + gap);
                            }
                        }

                        cumulatedLengths.Pop();
                        currentLength += maxLength;
                        length.Peek()[0] += maxLength;
                        break;
                    case '|':
                        length.Peek().Insert(0, 0);
                        break;
                    case '^':
                        cumulatedLengths.Push(0);
                        var start = new List<int>();
                        start.Add(0);
                        length.Push(start);
                        break;
                    case '(':
                        cumulatedLengths.Push(cumulatedLengths.Peek()+length.Peek()[0]);
                        start = new List<int>();
                        start.Add(0);
                        length.Push(start);
                        break;
                    case '$':
                        targetLength = length.Pop().Max();
                        break;
                    default:
                        length.Peek()[0]++;
                        break;
            ...
            Console.WriteLine($"Result : {targetLength}");
        }
