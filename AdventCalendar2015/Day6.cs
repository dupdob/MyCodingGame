...
namespace AdventCalendar2015
{
    public class Day6
    {
        ...
        public int TotalBrightness(string input = Input)
        {
            ...
            {
                for (var yIndex = 0; yIndex < lights.GetLength(0); yIndex++)
                {
                    result += lights[yIndex, xIndex];
                }
            }
        ...
        private static void Analyze<T>(string input, T[,] array, Func<T, T> toggle, Func<T, T> turnOn,
            Func<T, T> turnOff)
        {
            foreach (var line in input.Split('\n'))
            {
                ...
                var start = Extract(match.Groups[2]);
                var end = Extract(match.Groups[3]);

                var transform = match.Groups[1].Value switch
                {
                    "toggle" => toggle,
                    "turn on" => turnOn,
                    "turn off" => turnOff,
                    _ => throw new ApplicationException($"Line matching failed {line} on {match.Groups[1]}")
                };
