    ...
    public class Day7
    {
        public int FindSignal_a(string input = Input)
        {
            ...
            foreach (var line in input.Split('\n'))
            {
                ...
                state[matched.Groups[2].Value] = new Gate(matched.Groups[1].Value);
            }
        ...
        private class Gate
        {
            ...
            public void Force(ushort value)
            {
                _value = value;
            }
            private ushort GetValue(Capture capture, IReadOnlyDictionary<string, Gate> state)
            {
                if (ushort.TryParse(capture.Value, out var result))
                {
                    return result;
                }

                return state[capture.Value].Evaluate(state);
            }
            
            public ushort Evaluate(IReadOnlyDictionary<string, Gate> state)
            {
                if (_value.HasValue)
                {
                    return _value.Value;
                ...
                // check pattern
                if (matched.Groups[1].Success)
                {
                    _value = GetValue(matched.Groups[3], state);
                    if (matched.Groups[2].Success)
                    {
                        _value = (ushort) ~_value;
                    }
                }
                else
                {
                    ...
                    var x = GetValue(matched.Groups[5], state);
                    var y = GetValue(matched.Groups[7], state);
                    _value = matched.Groups[6].Value switch
                    {
                        "RSHIFT" => (ushort) (x >> y),
                        "LSHIFT" => (ushort) (x << y),
                        "AND" => (ushort) (x & y),
