...
namespace AdventCalendar2019
{
    public class Day3
    {
        private readonly Dictionary<Point, int> firstWire = new Dictionary<Point, int>();
        private readonly Dictionary<Point, int> secondWire = new Dictionary<Point, int>();

        public void Parse(string input = Input)
        {
            var lines = input.Split('\n');
            TracePath(lines[0], firstWire);
            TracePath(lines[1], secondWire);
        }

        public int FindCloserIntersection()
        {
            var origin  = new Point(0, 0);

            return (from point in firstWire.Keys where secondWire.ContainsKey(point) select point.ManhattanDist(origin)).Concat(new[] {int.MaxValue}).Min();
        }

        public int FindShortestPathToIntersection()
        {
            var dist = int.MaxValue;
            foreach (var pt in firstWire)
            {
                if (secondWire.ContainsKey(pt.Key))
                {
                    dist = Math.Min(pt.Value + secondWire[pt.Key], dist);
                }
            }
        ...
        private static void TracePath(string path, Dictionary<Point, int> points)
        {
            ...
            foreach (var segment in path.Split(','))
            {
                ...
                for (var i = 0; i < len; i++)
                {
                    point = point.Move(segment[0]);
                    if (!points.ContainsKey(point))
                       points.Add(point, dist);
                    dist++;
                }
            }
        }
